...
public class CompraSearchView extends javax.swing.JFrame {

    public CompraSearchView() {
        initComponents();
    }
    ...
    private void jbPesquisarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jbPesquisarActionPerformed
        ...
        List<Compra> compra = CompraController.getInstance().pesquisar(jtfPesquisar.getText());

        DefaultTableModel modelo = new DefaultTableModel();
        
        modelo.addColumn("Data");
        modelo.addColumn("id_fornecedor");
        modelo.addColumn("id_funcionario");
       
        for (Compra i : compra) {
            modelo.addRow(new Object[]{i.getId(), i.getData(), i.getFornecedor(), i.getFuncionario() });
        }
    ...
    private void jbExcluir2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jbExcluir2ActionPerformed
        ...
        if (linha > 0) {
            if (JOptionPane.showConfirmDialog(this, "Deseja deletar?", "Deletar registro", JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) {
                ...
                if (CompraController.getInstance().deletar(id)) {
    ...
    private void jbAlterar2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jbAlterar2ActionPerformed
        ...
        if (linha > 0)
        {
            ...
            Compra i = CompraController.getInstance().buscarID(id);

           CompraEditView view = new CompraEditView();
            view.setDados(i);
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new CompraSearchView().setVisible(true);
            }
        });
    }
